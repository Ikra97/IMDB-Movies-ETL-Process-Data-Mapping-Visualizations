...
package final_project.dim_crew_0_1;
...
/**
 * Job: dim_crew Purpose: <br>
 * Description:  <br>
 * @author gorle.g@northeastern.edu
 * @version 8.0.1.20230913_0925-patch
 * @status 
 */
public class dim_crew implements TalendJob {
	static {System.setProperty("TalendJob.log", "dim_crew.log");}

	

	
	private static org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager.getLogger(dim_crew.class);
...
public String getJob_name(){
	...
	private final String jobName = "dim_crew";
...
private final JobStructureCatcherUtils talendJobLog = new JobStructureCatcherUtils(jobName, "_orOisJDGEe6FmMo3PpyKlQ", "0.1");
		...
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(dim_crew.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class toScdStruct implements routines.system.IPersistableRow<toScdStruct> {
    final static byte[] commonByteArrayLock_FINAL_PROJECT_dim_crew = new byte[0];
    static byte[] commonByteArray_FINAL_PROJECT_dim_crew = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_FINAL_PROJECT_dim_crew.length) {
				if(length < 1024 && commonByteArray_FINAL_PROJECT_dim_crew.length == 0) {
   					commonByteArray_FINAL_PROJECT_dim_crew = new byte[1024];
				} else {
   					commonByteArray_FINAL_PROJECT_dim_crew = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_FINAL_PROJECT_dim_crew, 0, length);
			strReturn = new String(commonByteArray_FINAL_PROJECT_dim_crew, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_FINAL_PROJECT_dim_crew, 0, length);
			strReturn = new String(commonByteArray_FINAL_PROJECT_dim_crew, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_FINAL_PROJECT_dim_crew) {
...
public void tDBInput_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
 ...
 final boolean execStat = this.execStat;
		...
		org.slf4j.MDC.put("_subJobPid", "wCG4uR_" + subJobPidCounter.getAndIncrement());
...
toScdStruct toScd = new toScdStruct();
	...
	currentComponent="tDBSCD_1";
		...
		int tos_count_tDBSCD_1 = 0;
			...
			if(enableLogStash) {
				talendJobLog.addCM("tDBSCD_1", "MySqlLocal", "tMysqlSCD");
				talendJobLogProcess(globalMap);
			}
...
public int hashCode() {
if(hashCodeDirty) {
int prime = 31;hashCode = prime * hashCode + (nmconst == null ? 0 : nmconst.hashCode());
hashCodeDirty = false;
}
return hashCode;
}
...
private java.util.Date DI_CreateDate;
...
private String Name;
    ...
    String tableName_tDBSCD_1 = "dim_crew";
        ...
        java.sql.Connection connection_tDBSCD_1 = java.sql.DriverManager.getConnection(url_tDBSCD_1, context.MySqlLocal_Login, decryptedPassword_tDBSCD_1);
			...
			java.sql.DatabaseMetaData dbMetaData_tDBSCD_1 = connection_tDBSCD_1.getMetaData();
                ...
                while(rsTable_tDBSCD_1.next()) {
                    ...
                    if(table_tDBSCD_1.equalsIgnoreCase("dim_crew")) {
            ...
            if(!whetherExist_tDBSCD_1) {
                ...
                stmtCreate_tDBSCD_1.execute("CREATE TABLE `" + tableName_tDBSCD_1 + "`(`nmconst` VARCHAR(10)  ,`Name` VARCHAR(255)  ,`scd_start` DATETIME ,`DI_CreateDate` DATETIME ,`DI_WorkflowJobId` VARCHAR(8)  ,`scd_end` DATETIME ,`Crew_SK` INT(0)  AUTO_INCREMENT  not null ,primary key(`Crew_SK`))");
    ...
    String tmpValue_tDBSCD_1 = null;    
        String search_tDBSCD_1 = "SELECT `nmconst`, `DI_CreateDate`, `DI_WorkflowJobId`, `Name` FROM `" + tableName_tDBSCD_1 + "` WHERE `scd_end` IS NULL";
        ...
        while(resultSet_tDBSCD_1.next()) {
            ...
            SCDStruct_tDBSCD_1 row_tDBSCD_1 = new SCDStruct_tDBSCD_1();
                    if(resultSet_tDBSCD_1.getObject(1) != null) {
                        sk_tDBSCD_1.nmconst = resultSet_tDBSCD_1.getString(1);
                    }
                    ...
                    if(dateStr_tDBSCD_1 != null) {
                        if(!("0000-00-00").equals(dateStr_tDBSCD_1) && !("0000-00-00 00:00:00").equals(dateStr_tDBSCD_1)) {
                            row_tDBSCD_1.DI_CreateDate = new java.util.Date(resultSet_tDBSCD_1.getTimestamp(2).getTime());
                        } else {
                            row_tDBSCD_1.DI_CreateDate = (java.util.Date) dateYear0_tDBSCD_1.clone();
                        }
                    } else {
                        row_tDBSCD_1.DI_CreateDate = null;
                    }
                    if(resultSet_tDBSCD_1.getObject(3) != null) {
                        row_tDBSCD_1.DI_WorkflowJobId = resultSet_tDBSCD_1.getString(3);
                    }
                    if(resultSet_tDBSCD_1.getObject(4) != null) {
                        row_tDBSCD_1.Name = resultSet_tDBSCD_1.getString(4);
                    }
            cache_tDBSCD_1.put(sk_tDBSCD_1, row_tDBSCD_1);
        }
        resultSet_tDBSCD_1.close();
        statement_tDBSCD_1.close();
    String insertionSQL_tDBSCD_1 = "INSERT INTO `" + tableName_tDBSCD_1 + "`(`nmconst`, `DI_CreateDate`, `DI_WorkflowJobId`, `Name`, `scd_start`, `scd_end`) VALUES(?, ?, ?, ?, ?, ?)";
    java.sql.PreparedStatement insertionStatement_tDBSCD_1 = connection_tDBSCD_1.prepareStatement(insertionSQL_tDBSCD_1);
        ...
        String updateSQLForType1_tDBSCD_1 = "UPDATE `" + tableName_tDBSCD_1 + "` SET `DI_CreateDate` = ?, `DI_WorkflowJobId` = ? WHERE `nmconst` = ?";
        java.sql.PreparedStatement updateForType1_tDBSCD_1 = connection_tDBSCD_1.prepareStatement(updateSQLForType1_tDBSCD_1);        
        String updateSQLForType2_tDBSCD_1 = "UPDATE `" + tableName_tDBSCD_1 + "` SET `scd_end` = ? WHERE `nmconst` = ? AND `scd_end` IS NULL";
 ...
 */
	...
	currentComponent="tDBInput_1";
		...
		int tos_count_tDBInput_1 = 0;
            ...
            if (log.isDebugEnabled()) {
                class BytesLimit65535_tDBInput_1{
                    ...
                    log4jParamters_tDBInput_1.append("Parameters:");
                        ...
                        log4jParamters_tDBInput_1.append(" | ");
                            log4jParamters_tDBInput_1.append("TABLE" + " = " + "\"stg_crew_name\"");
                        log4jParamters_tDBInput_1.append(" | ");
                            log4jParamters_tDBInput_1.append("QUERYSTORE" + " = " + "\"\"");
                        log4jParamters_tDBInput_1.append(" | ");
                            log4jParamters_tDBInput_1.append("QUERY" + " = " + "\"SELECT nconst, primaryName, DI_Create_DT  FROM stg_crew_name;\"");
                        log4jParamters_tDBInput_1.append(" | ");
                        ...
                        log4jParamters_tDBInput_1.append(" | ");
                            log4jParamters_tDBInput_1.append("TRIM_COLUMN" + " = " + "[{TRIM="+("false")+", SCHEMA_COLUMN="+("nconst")+"}, {TRIM="+("false")+", SCHEMA_COLUMN="+("primaryName")+"}, {TRIM="+("false")+", SCHEMA_COLUMN="+("DI_Create_DT")+"}]");
                        log4jParamters_tDBInput_1.append(" | ");
			...
			if(enableLogStash) {
				talendJobLog.addCM("tDBInput_1", "stg_crew_name", "tMysqlInput");
				talendJobLogProcess(globalMap);
			}
	...
	final String decryptedPassword_tDBInput_1 = context.MySqlLocal_Password; 
        ...
        String url_tDBInput_1 = "jdbc:mysql://" + context.MySqlLocal_Server + ":" + context.MySqlLocal_Port + "/" + context.MySqlLocal_Database + "?" + properties_tDBInput_1;
		    ...
		    String dbquery_tDBInput_1 = "SELECT nconst, primaryName, DI_Create_DT  FROM stg_crew_name;";
        	...
        	row1.nconst = routines.system.JDBCUtil.getString(rs_tDBInput_1, 1, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 2) {
								row1.primaryName = null;
							} else {
	                         		
        	row1.primaryName = routines.system.JDBCUtil.getString(rs_tDBInput_1, 2, false);
		                    }
 ...
 */
	...
	currentComponent="tUnite_1";
					...
					if(runStat.update(execStat,enableLogStash,iterateId,1,1
							...
							,"row1","tDBInput_1","stg_crew_name","tMysqlInput","tUnite_1","tUnite_1","tUnite"
...
toScd_tmp.nmconst = row3.nconst;
toScd_tmp.Name = row3.primaryName;
toScd_tmp.scd_start = row3.DI_Create_DT;
toScd_tmp.DI_CreateDate = TalendDate.getCurrentDate();
...
if(toScd != null) { 
	...
	currentComponent="tDBSCD_1";
			...
			if(runStat.update(execStat,enableLogStash,iterateId,1,1
					...
					,"toScd","tMap_1","tMap_1","tMap","tDBSCD_1","MySqlLocal","tMysqlSCD"
	...
	try {
        lookUpKey_tDBSCD_1 = new SCDSK_tDBSCD_1();
            lookUpKey_tDBSCD_1.nmconst = toScd.nmconst;
    ...
    if(lookUpValue_tDBSCD_1 == null) {
            lookUpValue_tDBSCD_1 = new SCDStruct_tDBSCD_1();
                    ...
                    if(toScd.nmconst == null) {
insertionStatement_tDBSCD_1.setNull(1, java.sql.Types.VARCHAR);
} else {
insertionStatement_tDBSCD_1.setString(1, toScd.nmconst);
}

                    if(toScd.DI_CreateDate == null) {
insertionStatement_tDBSCD_1.setNull(2, java.sql.Types.DATE);
} else {
if(toScd.DI_CreateDate != null) {
date_tDBSCD_1 = toScd.DI_CreateDate.getTime();
...
insertionStatement_tDBSCD_1.setTimestamp(2, new java.sql.Timestamp(date_tDBSCD_1));
}
} else {
insertionStatement_tDBSCD_1.setNull(2, java.sql.Types.DATE);
}
...
insertionStatement_tDBSCD_1.setString(3, toScd.DI_WorkflowJobId);
}

                    if(toScd.Name == null) {
insertionStatement_tDBSCD_1.setNull(4, java.sql.Types.VARCHAR);
} else {
insertionStatement_tDBSCD_1.setString(4, toScd.Name);
}
...
insertionStatement_tDBSCD_1.setNull(5, java.sql.Types.DATE);
        ...
        nb_line_inserted_tDBSCD_1 += insertionStatement_tDBSCD_1.executeUpdate();
            ...
            if(!cu_tDBSCD_1.scdEquals(lookUpValue_tDBSCD_1.DI_CreateDate,toScd.DI_CreateDate) || (lookUpValue_tDBSCD_1.DI_WorkflowJobId == null && toScd.DI_WorkflowJobId!= null) || (lookUpValue_tDBSCD_1.DI_WorkflowJobId != null && !lookUpValue_tDBSCD_1.DI_WorkflowJobId.equals(toScd.DI_WorkflowJobId))) {
                    if(toScd.DI_CreateDate == null) {
...
updateForType1_tDBSCD_1.setTimestamp(1, new java.sql.Timestamp(date_tDBSCD_1));
}
} else {
updateForType1_tDBSCD_1.setNull(1, java.sql.Types.DATE);
}
...
updateForType1_tDBSCD_1.setString(2, toScd.DI_WorkflowJobId);
}

                    if(toScd.nmconst == null) {
updateForType1_tDBSCD_1.setNull(3, java.sql.Types.VARCHAR);
} else {
updateForType1_tDBSCD_1.setString(3, toScd.nmconst);
}

                nb_line_update_tDBSCD_1 += updateForType1_tDBSCD_1.executeUpdate();
            }
            if((lookUpValue_tDBSCD_1.Name == null && toScd.Name!= null) || (lookUpValue_tDBSCD_1.Name != null && !lookUpValue_tDBSCD_1.Name.equals(toScd.Name))) {
                    if(toScd.scd_start == null) {
...
updateForType2_tDBSCD_1.setString(2, toScd.nmconst);
}
...
globalMap.put("tDBSCD_1_ERROR_MESSAGE",e.getMessage());
                ...
                lookUpValue_tDBSCD_1.DI_CreateDate = toScd.DI_CreateDate;
                lookUpValue_tDBSCD_1.DI_WorkflowJobId = toScd.DI_WorkflowJobId;
                lookUpValue_tDBSCD_1.Name = toScd.Name;
 ...
 */
	...
	currentComponent="tDBInput_2";
		...
		int tos_count_tDBInput_2 = 0;
            ...
            if (log.isDebugEnabled()) {
                class BytesLimit65535_tDBInput_2{
                    ...
                    log4jParamters_tDBInput_2.append("Parameters:");
                        ...
                        log4jParamters_tDBInput_2.append(" | ");
                            log4jParamters_tDBInput_2.append("TABLE" + " = " + "\"new_name_basics\"");
                        log4jParamters_tDBInput_2.append(" | ");
                            log4jParamters_tDBInput_2.append("QUERYSTORE" + " = " + "\"\"");
                        log4jParamters_tDBInput_2.append(" | ");
                            log4jParamters_tDBInput_2.append("QUERY" + " = " + "\"SELECT nconst, primaryName, DI_CreatedDate  FROM new_name_basics;\"");
                        log4jParamters_tDBInput_2.append(" | ");
                        ...
                        log4jParamters_tDBInput_2.append(" | ");
                            log4jParamters_tDBInput_2.append("TRIM_COLUMN" + " = " + "[{TRIM="+("false")+", SCHEMA_COLUMN="+("nconst")+"}, {TRIM="+("false")+", SCHEMA_COLUMN="+("primaryName")+"}, {TRIM="+("false")+", SCHEMA_COLUMN="+("DI_Create_DT")+"}]");
                        log4jParamters_tDBInput_2.append(" | ");
			...
			if(enableLogStash) {
				talendJobLog.addCM("tDBInput_2", "stg_crew_name_v2", "tMysqlInput");
				talendJobLogProcess(globalMap);
			}
	...
	final String decryptedPassword_tDBInput_2 = context.MySqlLocal_Password; 
        ...
        String url_tDBInput_2 = "jdbc:mysql://" + context.MySqlLocal_Server + ":" + context.MySqlLocal_Port + "/" + context.MySqlLocal_Database + "?" + properties_tDBInput_2;
		    ...
		    String dbquery_tDBInput_2 = "SELECT nconst, primaryName, DI_CreatedDate  FROM new_name_basics;";
        	...
        	row2.nconst = routines.system.JDBCUtil.getString(rs_tDBInput_2, 1, false);
		                    }
							if(colQtyInRs_tDBInput_2 < 2) {
								row2.primaryName = null;
							} else {
	                         		
        	row2.primaryName = routines.system.JDBCUtil.getString(rs_tDBInput_2, 2, false);
		                    }
 ...
 */
	...
	currentComponent="tUnite_1";
					...
					if(runStat.update(execStat,enableLogStash,iterateId,1,1
							...
							,"row2","tDBInput_2","stg_crew_name_v2","tMysqlInput","tUnite_1","tUnite_1","tUnite"
...
// for output
			...
			row3.nconst = row2.nconst;								
			row3.primaryName = row2.primaryName;								
...
globalMap.put("tUnite_1_NB_LINE", nb_line_tUnite_1);
				...
				if(enableLogStash) {
				 		...
				 		if(runStat.log(resourceMap,iterateId,"row1",2,0,
				 			"tDBInput_1","stg_crew_name","tMysqlInput","tUnite_1","tUnite_1","tUnite","output")) {
							talendJobLogProcess(globalMap);
						}
					
				 		if(runStat.log(resourceMap,iterateId,"row2",2,0,
				 			"tDBInput_2","stg_crew_name_v2","tMysqlInput","tUnite_1","tUnite_1","tUnite","output")) {
							talendJobLogProcess(globalMap);
						}
 ...
 */
    ...
    globalMap.put("tDBSCD_1_NB_LINE_REJECTED",nb_line_rejected_tDBSCD_1);
			 		...
			 		if(runStat.updateStatAndLog(execStat,enableLogStash,resourceMap,iterateId,"toScd",2,0,
			 			"tMap_1","tMap_1","tMap","tDBSCD_1","MySqlLocal","tMysqlSCD","output")) {
						talendJobLogProcess(globalMap);
					}
...
public void tPrejob_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
 ...
 final boolean execStat = this.execStat;
		...
		org.slf4j.MDC.put("_subJobPid", "yCLI1H_" + subJobPidCounter.getAndIncrement());
...
public void tRunJob_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
 ...
 final boolean execStat = this.execStat;
		...
		org.slf4j.MDC.put("_subJobPid", "zMly3p_" + subJobPidCounter.getAndIncrement());
...
public void tPostjob_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
 ...
 final boolean execStat = this.execStat;
		...
		org.slf4j.MDC.put("_subJobPid", "Bzf9NN_" + subJobPidCounter.getAndIncrement());
...
public void tRunJob_2Process(final java.util.Map<String, Object> globalMap) throws TalendException {
 ...
 final boolean execStat = this.execStat;
		...
		org.slf4j.MDC.put("_subJobPid", "96gHmn_" + subJobPidCounter.getAndIncrement());
...
public void talendJobLogProcess(final java.util.Map<String, Object> globalMap) throws TalendException {
 ...
 final boolean execStat = this.execStat;
		...
		org.slf4j.MDC.put("_subJobPid", "7DcgmN_" + subJobPidCounter.getAndIncrement());
 ...
 */
    ...
    public static void main(String[] args){
        final dim_crew dim_crewClass = new dim_crew();

        int exitCode = dim_crewClass.runJobInTOS(args);
	        if(exitCode==0){
		        log.info("TalendJob: 'dim_crew' - Done.");
	        }
    ...
    public int runJobInTOS(String[] args) {
        ...
        enableLogStash = "true".equalsIgnoreCase(System.getProperty("audit.enabled"));
	        ...
	        getjobInfo();
			log.info("TalendJob: 'dim_crew' - Start.");
                ...
                org.slf4j.MDC.put("_jobRepositoryId","_orOisJDGEe6FmMo3PpyKlQ");
                org.slf4j.MDC.put("_compiledAtTimestamp","2023-12-04T02:08:28.022185Z");
        ...
        try {
            ...
            java.io.InputStream inContext = dim_crew.class.getClassLoader().getResourceAsStream("final_project/dim_crew_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = dim_crew.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
        ...
        resumeUtil.addLog("JOB_STARTED", "JOB:" + jobName, parent_part_launcher, Thread.currentThread().getId() + "", "","","","",resumeUtil.convertToJsonText(context,ContextProperties.class,parametersToEncrypt));
            ...
            log.info("TalendJob: 'dim_crew' - Started.");
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : dim_crew");
        }
		if(enableLogStash) {
...
if (execStat) {
    ...
    resumeUtil.flush();
        ...
        log.info("TalendJob: 'dim_crew' - Finished - status: " + status + " returnCode: " + returnCode );
...
/************************************************************************************************
 *     174642 characters generated by Talend Cloud Data Fabric 
 *     on the December 3, 2023 at 9:08:28 PM EST
 ************************************************************************************************/